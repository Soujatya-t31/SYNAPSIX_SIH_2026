...
1. RELEVANCE CHECK (real trained model): a genuine pretrained YOLOv8 (COCO weights)
   ...
   The model loads in-process (lazy-loaded on first use) from the bundled
   weights file in backend/models/yolov8n.pt. Falls back to a local OpenCV
   skin-tone heuristic automatically if the weights can't be loaded for any
   reason (e.g. no internet access to fetch them and the bundled file is
   missing) — the app never hard-depends on the real model being available.
...
# ---------------------------------------------------------------------------
# Real YOLOv8 (COCO-pretrained) — lazy-loaded in-process so the module still
# imports fine in environments without internet access to fetch the weights;
# falls back to the OpenCV skin-tone heuristic if the model can't be loaded.
# ---------------------------------------------------------------------------
_yolo_model = None
...
def _yolo_relevance_check(path: str, claimed_category: str):
    """Returns (person_dominant, coco_hits) using the real pretrained detector.
    person_dominant=True means a person fills enough of the frame that this is
    very likely a selfie, not an infrastructure photo. Returns None (signal to
    fall back to the local heuristic) if the model can't be loaded or the
    prediction call fails for any reason — a missing/broken model should never
    break complaint submission."""
    model = _get_yolo()
    if model is None:
        return None
    try:
...
def _dark_irregular_score(img_bgr):
    ...
    for c in contours:
        ...
        # Noise-spec filter was 0.1% of the frame, which let almost any patch
        # of edge noise through as a "defect" — bumped to 0.4% so a defect
        # actually has to cover a visible patch of the photo.
        ...
        # Raised from 0.15 alongside the larger area filter, for the same reason.
    ...
    # The 0.35 floor here used to apply even with zero real defect signal, and
    # edge_density was added to confidence uncapped-on-the-low-end — so a
    # noisy/blurry/irrelevant photo (which has HIGH edge density everywhere,
    # not a real localized defect) scored just as well as an actual pothole.
    # That, plus the floor, is why verification basically always said "yes".
    # Fixed base is now much lower, and very high whole-frame edge density is
    # treated as a sign of noise/clutter rather than a clean defect shot, so
    # it stops contributing once it gets that high.
    edge_signal = edge_density if edge_density <= 0.35 else max(0.0, 0.35 - (edge_density - 0.35))
    confidence = 0.12 + avg_defect_quality * 0.85 + edge_signal * 0.4
    # A single "defect" that swallows most of the frame is more likely a
    # noisy/misread photo than one real localized pothole/crack — real
    # defects are typically a fraction of the shot, not nearly all of it.
    if damage_area_pct > 60:
        confidence *= 0.5
    if defect_count == 0:
        confidence *= 0.3
...
def _organic_color_score(img_bgr):
    ...
    # Same fix as _dark_irregular_score: lower base, so a photo with barely
    # any clutter doesn't already start near the "valid" threshold.
    confidence = min(0.95, 0.15 + min(damage_area_pct / 60.0, 0.55) + (0.1 if defect_count else 0))
    return {
...
def analyze_image(path: str, claimed_category: str) -> dict:
    ...
    # Raised from 0.35 to match the recalibrated confidence formulas above —
    # otherwise the lower base scores would just make everything "invalid"
    # instead of everything "valid".
    valid = result["confidence"] >= 0.45 and result["defect_count"] > 0
    notes = (
