...
def startup():
    # Postgres: schema creation is fully idempotent (CREATE TABLE IF NOT EXISTS
    # throughout), so just run it every startup — no local file to check for.
    # SQLite: only run the (destructive-if-forced) init on first run, i.e. when
    # the local .db file doesn't exist yet.
    if database.USE_POSTGRES:
        database.init_db()
    elif not os.path.exists(database.DB_PATH):
...
def run_pipeline(conn, category, description, citizen_severity, lat, lon, image_path):
    landmarks = geo_utils.get_landmarks_near(conn, lat, lon)
...
def get_complaint(complaint_id: str):
    ...
    try:
        ...
        landmarks = geo_utils.get_landmarks_near(conn, d["latitude"], d["longitude"])
...
def upvote_complaint(complaint_id: str, x_voter_key: str = Header(None)):
    ...
    try:
        ...
        landmarks = geo_utils.get_landmarks_near(conn, row["latitude"], row["longitude"])
...
def update_status(
    ...
    JanDrishti bell regardless of email setup), and a real email is attempted
...
def get_stats():
    ...
    try:
        ...
        # Resolved count per category — pairs with by_category above to drive the
        # government dashboard's "universal counter" (every category, reported vs
        # resolved), not just the categories that happen to have an active task.
        by_category_resolved = {r["category"]: r["c"] for r in conn.execute(
            "SELECT category, COUNT(*) c FROM complaints WHERE is_primary=1 AND status = 'RESOLVED' GROUP BY category"
        ).fetchall()}
        ...
        # Every known category, not just ones with at least one report, so the
        # dashboard's category counter always shows the full, fixed set (zeros
        # included) rather than silently dropping categories with no activity yet.
        all_categories = list(priority_engine.DEPARTMENT_MAP.keys())
        return {
            ...
            "by_category_resolved": by_category_resolved,
...
def seed(n: int = 150, reset: bool = True):
    # Guardrail: this endpoint is unauthenticated (fine for local hackathon dev),
    # but it must never be reachable against the live shared Postgres database —
    # anyone could hit it and flood production with fake demo complaints. It only
    # works in local SQLite mode; disabled entirely once DATABASE_URL is set.
    if database.USE_POSTGRES:
        raise HTTPException(403, "Seeding is disabled when running against the shared production database. Run seed_data.py locally against SQLite instead.")
    import seed_data
