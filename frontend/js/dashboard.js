...
// ---- View toggle (Priority Queue <-> Departments) ----
...
// ---- Map setup ----
function initMap() {
  map = L.map("map", { zoomControl: true, attributionControl: true }).setView([22.555, 88.37], 12);
...
async function loadStats() {
  ...
  // Total items currently represented by the four workflow phases.
  const phases = ["RECEIVED", "ASSIGNED", "IN_PROGRESS", "RESOLVED"];
  document.getElementById("statWorkflow").textContent = phases.reduce(
    (total, status) => total + (s.by_status?.[status] || 0), 0
  );
}
...
document.querySelectorAll(".d-stat").forEach((el) => {
  el.addEventListener("click", () => {
    ...
    if (lvl === "__affected" || lvl === "__workflow") return;
  ...
  overlay.innerHTML = `
    <div class="d-detail">
      ...
      <div class="d-info-grid" style="margin-top:14px;">
        ...
        <div class="d-info-item"><div class="l">SLA target</div><div class="v">${c.sla_hours} hrs</div></div>
