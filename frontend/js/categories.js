...
// Shared "target response" formatter — sla_hours from the backend is always
// whole hours (see priority_engine.SLA_HOURS: 24/72/168/336), so days is
// always exact, but this stays defensive in case that ever changes.
function formatSla(hours) {
  if (hours === null || hours === undefined) return "—";
  const days = hours / 24;
  const daysLabel = Number.isInteger(days) ? `${days} day${days === 1 ? "" : "s"}` : `${days.toFixed(1)} days`;
  return `${hours} hrs (${daysLabel})`;
}
