...
document.getElementById("quickNextBtn").addEventListener("click", () => {
  ...
  try { sessionStorage.setItem("civicvoice_quick_category", quickSelected); } catch (e) {}
...
function renderMap(rows) {
  ...
  if (!leafletMap) {
    leafletMap = L.map(el, { zoomControl: false, attributionControl: false }).setView([22.555, 88.37], 11);
    L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", { maxZoom: 19 }).addTo(leafletMap);
  }
  const colors = { Critical: "#C1392B", High: "#E0912B", Medium: "#2E6E8E", Low: "#5C7A63" };
  const pts = [];
  rows.forEach((c) => {
    if (c.latitude == null || c.longitude == null) return;
    const color = colors[c.priority_level] || colors.Low;
    L.circleMarker([c.latitude, c.longitude], {
      radius: 6, color, fillColor: color, fillOpacity: 0.85, weight: 1.5,
    })
      .bindTooltip(`${c.category} — ${c.priority_level}${c.address ? " — " + c.address : ""}`)
      .addTo(leafletMap);
